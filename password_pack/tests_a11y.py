...
def run_a11y(base_url, contract, outdir, chromium_path=None):
    os.makedirs(outdir, exist_ok=True)
    results = {"violations": [], "status":"PASS"}
    limits = contract.get("a11y", {}); max_serious = int(limits.get("max_serious", 0)); max_critical = int(limits.get("max_critical", 0))
    ...
    with sync_playwright() as pw:
        ...
        serious = sum(1 for v in violations if v.get("impact")=="serious"); critical = sum(1 for v in violations if v.get("impact")=="critical")
        if serious > max_serious or critical > max_critical: results["status"]="FAIL"
        context.close(); browser.close()

    with open(os.path.join(outdir,"a11y.json"),"w",encoding="utf-8") as f: json.dump(results,f,indent=2)
    with open(os.path.join(outdir,"a11y.txt"),"w",encoding="utf-8") as f: f.write(json.dumps(results,indent=2))
    return results
