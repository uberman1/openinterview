...
def run_workflows(base_url, contract, outdir, chromium_path=None):
            ...
            try:
                for step in wf.get("steps", []):
                    if "visit" in step:
                        ...
                        sel = step["wait_for"]["selector"]; timeout = step["wait_for"].get("timeout_ms", 2000)
    ...
    with open(os.path.join(outdir,"behavior.json"),"w",encoding="utf-8") as f:
        json.dump(results,f,indent=2)
    with open(os.path.join(outdir,"behavior.txt"),"w",encoding="utf-8") as f:
