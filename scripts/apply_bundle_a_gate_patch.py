...
if "bundle_a" in src or "bundle_a/run_bundle_a_tests.py" in src:
...
# Look for PACKS or STEPS list
pattern = re.compile(r"((?:PACKS|STEPS)\s*=\s*\[)(.*?)(\])", re.S)
m = pattern.search(src)
if not m:
    print("Could not locate PACKS or STEPS list. Please add manually.", file=sys.stderr)
    sys.exit(2)
