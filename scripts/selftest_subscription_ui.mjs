...
(async function run(){
  ...
  const res2 = await request(app).get('/billing').redirects(1);
  ok(res2.status===200, '/billing redirects to subscription page');
  console.log(`\nSubscription UI v2 test: ${pass} passed, ${fail} failed`);
