...
// Paid vs free tier — only for buttons (View Plans / Manage in Stripe), not for display name
function isFreePlan(code) {
  return (code || 'free').toLowerCase() === 'free';
}
...
async function openStripePortal(button) {
  ...
  try {
    ...
    if (typeof window.trackGAEvent === 'function') {
      window.trackGAEvent('checkout_started', { source: 'subscription_page' });
    }
...
function buildCompleteContent(data) {
  ...
  const planDisplayName = c.planName || plan;
  const cancelDateLabel = formatLocaleDate(c.cancelAtPeriodEnd);
  const cancelNotice = cancelDateLabel && !isFreePlan(plan)
    ? `<p class="mt-2 text-sm text-amber-600 dark:text-amber-400">
         Your subscription cancels on ${cancelDateLabel}.
       </p>`
    : '';

  contentContainer.innerHTML = `
    <div id="sub-plan-compliance-banner" class="mb-6 hidden"></div>
    <!-- Current Plan -->
    ...
    <div class="flex justify-end pt-4 animate-[fadeIn_0.5s_ease-out_0.3s_both]">
      <button id="manage-stripe-btn" class="bg-primary text-white dark:bg-white dark:text-primary px-6 py-3 rounded-lg text-sm font-semibold hover:bg-primary/90 dark:hover:bg-white/90 transition-colors">
        ${isFreePlan(plan) ? 'View Plans' : 'Manage in Stripe'}
      </button>
    </div>
  `;

  if (window.PlanComplianceBanner) {
    window.PlanComplianceBanner.mount(
      data.compliance,
      contentContainer.querySelector('#sub-plan-compliance-banner')
    );
  }
