...
(function initProfileEdit() {
  ...
  async function loadProfile() {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        console.log(`[profile-edit] Loading profile (attempt ${attempt}/${maxAttempts}):`, currentProfileId);
        const res = await fetch(`/api/profiles/${currentProfileId}`, { credentials: 'include' });
        ...
        if (res.status === 401) {
          ...
          window.location.href = `/login-page.html?returnTo=${encodeURIComponent(returnUrl)}`;
          return;
        }

        if (res.status === 403) {
          console.warn('[profile-edit] Current user cannot access this profile; redirecting to dashboard');
          hidePageLoading();
          showErrorModal('This profile belongs to another account. Redirecting to your dashboard.');
          setTimeout(() => {
        ...
        if (res.status === 429 && attempt < maxAttempts) {
          // Bot/rate protection: wait and retry instead of failing instantly.
          const retryAfter = res.headers.get('Retry-After');
          const retryMs = retryAfter ? Math.ceil(Number(retryAfter) * 1000) : Math.min(1000 * Math.pow(2, attempt), 8000);
          console.warn('[profile-edit] Hit rate limit (429). Retrying in', retryMs, 'ms');
    ...
    console.error('[profile-edit] Exhausted loadProfile retries. Last status:', lastStatus);
    hidePageLoading();
    showErrorModal('Failed to load profile data');
  }
  ...
  function populateProfile(profile) {
    ...
    const smsEnabled = profile.contact?.smsNotificationsEnabled === true || profile.contact?.sms_notifications_enabled === true;
    const smsCheckbox = document.getElementById('input-sms-enabled');
    if (smsCheckbox) {
      smsCheckbox.checked = !!smsEnabled;
    }

    // Pre-check attestation if previously accepted
    const attestationCheckbox = document.getElementById('attestation-checkbox');
    if (attestationCheckbox && profile.contact?.profile_attestation_accepted === true) {
      attestationCheckbox.checked = true;
    }
  ...
  function reconstructAvailability(availability) {
    ...
    if (availability.rules) {
      ...
      if (minNoticeEl) {
        ...
        const parsedMinNotice = Number(r.minNoticeHours);
        const val = Number.isFinite(parsedMinNotice) ? parsedMinNotice : 24;
      ...
      try {
        ...
        if (res.ok) {
          ...
          setTimeout(() => {
            ...
            window.trackGAEvent('video_uploaded', { source: 'profile_edit' });
  ...
  function initRulesBindings() {
    ...
    if (noticeSelect) {
      noticeSelect.addEventListener('change', (e) => {
        ...
        const val = Number.isFinite(parsed) ? parsed : 24;
  ...
  function collectFormData() {
    ...
    return {
      ...
      contact: {
        ...
        smsNotificationsEnabled: !!document.getElementById('input-sms-enabled')?.checked,
        ...(document.getElementById('attestation-checkbox')?.checked
          ? { profile_attestation_accepted: true, profile_attestation_timestamp: new Date().toISOString() }
          : {})
      },
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTimeout(() => {
          ...
          window.trackGAEvent('resume_uploaded', { source: 'profile_edit' });
  ...
  async function saveProfile() {
    ...
    const attestationCheckbox = document.getElementById('attestation-checkbox');
    const attestationError = document.getElementById('attestation-error');
    if (attestationCheckbox && !attestationCheckbox.checked) {
      hideSaveLoading();
      if (attestationError) attestationError.classList.remove('hidden');
      attestationCheckbox.scrollIntoView({ behavior: 'smooth', block: 'center' });
      return;
    }
    if (attestationError) attestationError.classList.add('hidden');
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Preserve widget=1 when in widget flow so owner_preview shows Submit instead of Share
        setTimeout(() => {
          const isWidget =
            new URLSearchParams(window.location.search).get('widget') === '1' ||
            sessionStorage.getItem('oi_widget') === '1';
          const widgetParam = isWidget ? '&widget=1' : '';
          window.location.href = `/owner_preview.html?id=${currentProfileId}${widgetParam}`;
        }, 1500);
      } else {
        hideSaveLoading();
        ...
        if (res.status === 403) {
          showErrorModal(err.error || 'This profile belongs to another account. Redirecting to your dashboard.');
          setTimeout(() => {
            window.location.href = '/dashboard.html';
          }, 1200);
          return;
