...
(async function initDashboard() {
  ...
  function renderDashboardStatsAndInterviews(data) {
    ...
    if (statsContainer) {
      ...
      statsContainer.innerHTML = `
        <div class="flex flex-col gap-1 rounded border border-primary/10 bg-white p-4 dark:border-white/10 dark:bg-primary animate-[fadeIn_0.5s_ease-out]">
          ...
          <p class="text-sm text-primary/60 dark:text-white/60">Active Profile Openinterviews’ (Published)</p>
        </div>
      ...
      if (data.profiles.length === 0) {
        interviewsTableBody.innerHTML = `
                <tr class="animate-[fadeIn_0.5s_ease-out]">
                    <td colspan="7" class="px-6 py-4 text-center text-sm text-primary/60 dark:text-white/60">
                        No interviews found. Create a new one!
                    </td>
                </tr>
            `;
      } else {
        const sorted = data.profiles.slice().sort((a, b) => {
          const aPublished = a.visibility === 'public' ? 0 : 1;
          const bPublished = b.visibility === 'public' ? 0 : 1;
        ...
        sorted.forEach((profile, index) => {
          ...
          const views = profile.viewCount ?? profile.view_count ?? 0;
          const vis = profile.visibility || 'private';
          const handle = (profile.publicHandle || '').replace(/'/g, "\\'");
          ...
          row.innerHTML = `
                  ...
                  <td class="px-6 py-4 text-sm text-primary/60 dark:text-white/60">${views}</td>
                  <td class="px-6 py-4">
                    <div class="dashboard-publish-cell flex min-h-[1.25rem] items-center gap-2">
                      <input type="checkbox" class="dashboard-publish-toggle dashboard-toggle shrink-0"
                      ${isShared ? 'checked' : ''}
                      data-profile-id="${profile.id}"
                      onchange="handleDashboardPublishToggle(event)"
                      aria-label="Publish profile (public link)" />
                      <span class="dashboard-publish-loading hidden h-4 w-4 shrink-0 rounded-full border-2 border-primary/20 border-t-primary dark:border-white/20 dark:border-t-white animate-spin" aria-hidden="true"></span>
                    </div>
                  </td>
                  ...
                  <td class="px-6 py-4 text-sm text-right">
                    <div class="flex items-center justify-end gap-2">
                        <button class="flex h-8 w-8 items-center justify-center rounded-full bg-primary/5 text-primary hover:bg-primary/10 dark:bg-white/10 dark:text-white dark:hover:bg-white/20" onclick="openOwnerPreview('${profile.id}','${handle}')" title="Open owner preview" aria-label="Open owner preview">
                            <span class="material-symbols-outlined text-base">person</span>
                        </button>
                        <button class="flex h-8 w-8 items-center justify-center rounded hover:bg-primary/10 dark:hover:bg-white/10 text-primary/60 hover:text-primary dark:text-white/60 dark:hover:text-white" onclick="shareProfile('${profile.id}', '${vis}', '${handle}')" title="Share">
                        ...
                        <button type="button" data-delete-profile="${profile.id}" class="flex h-8 w-8 items-center justify-center rounded hover:bg-red-500/10 text-red-500 hover:text-red-700" onclick="deleteProfile('${profile.id}')" title="Delete" aria-label="Delete profile">
  ...
  window.__renderDashboardStatsAndInterviews = renderDashboardStatsAndInterviews;
  ...
  try {
    ...
    renderDashboardStatsAndInterviews(data);

    if (window.PlanComplianceBanner) {
      window.PlanComplianceBanner.mount(data.compliance);
    }
    ...
    (function() {
      ...
      wrapper.id = 'dashboard-rotating-banner';
      wrapper.style.cssText = 'position:relative;width:100%;overflow:hidden;border-radius:10px;line-height:0;';
      const img = document.createElement('img');
      img.src = '/images/banner-which-4.png';
      img.alt = 'OpenInterview';
      ...
      if (mainContent) {
        mainContent.insertBefore(wrapper, mainContent.children[1]);
      }
    })();
...
function openDashboardDeleteModal(profileId) {
  ...
  if (!modal) {
    if (confirm('Are you sure you want to delete this interview? This will delete all associated videos and resumes and release your storage/credits.')) {
      window.executeDashboardProfileDelete(profileId);
    }
    return;
  }
  ...
  const confirmBtn = modal.querySelector('[data-action="confirm-delete-modal"]');
  confirmBtn?.focus();
}
...
(function wireDashboardDeleteModal() {
  const modal = document.getElementById('dashboard-delete-modal');
  if (!modal) return;

  modal.querySelector('[data-action="close-delete-modal"]')?.addEventListener('click', closeDashboardDeleteModal);
  modal.querySelector('[data-action="cancel-delete-modal"]')?.addEventListener('click', closeDashboardDeleteModal);
  modal.querySelector('[data-action="confirm-delete-modal"]')?.addEventListener('click', () => {
    const id = modal.dataset.profileId;
    if (!id) return;
    closeDashboardDeleteModal();
    window.executeDashboardProfileDelete(id);
  });

  modal.addEventListener('click', (e) => {
    if (e.target === modal) closeDashboardDeleteModal();
  });
...
window.deleteProfile = function (profileId) {
  openDashboardDeleteModal(profileId);
};

window.executeDashboardProfileDelete = async function (profileId) {
  const btn = document.querySelector(`button[data-delete-profile="${CSS.escape(profileId)}"]`);
...
window.openOwnerPreview = function(profileId, handle) {
    if (!profileId) return;
    const isMobile = window.matchMedia('(pointer: coarse)').matches || window.innerWidth <= 768;
    if (isMobile && handle) {
        window.location.href = `/u/${encodeURIComponent(handle)}`;
    } else {
        window.location.href = `/owner_preview.html?id=${encodeURIComponent(profileId)}`;
    }
};
...
// Handle upgrade button click — sale banner starts Stripe checkout via env plan code
window.handleUpgradeClick = async function(e, btn) {
    e.preventDefault();

    if (btn.dataset.saleCheckout === '1') {
        const fallbackHref = btn.getAttribute('href') || '/subscription.html';
        const originalText = btn.textContent;
        btn.style.pointerEvents = 'none';
        btn.textContent = 'Processing...';

        try {
            const configRes = await fetch('/api/dashboard/sale-plan', { credentials: 'include' });
            const config = await configRes.json();
            const planId = config?.planCode;
            if (!planId) {
            ...
            if (data.checkoutUrl) {
                if (typeof window.trackGAEvent === 'function') {
                    window.trackGAEvent('checkout_started', { source: 'dashboard_sale_banner', plan: planId });
                }
            ...
            console.error('[dashboard] Sale checkout error:', err);
...
window.copyShareLink = async function() {
  ...
  try {
    ...
    window.trackGAEvent('profile_shared', { source: 'share_action' });
  } catch (error) {
...
window.shareProfile = function(profileId, visibility, publicHandle) {
    if (window.ShareModalController) {
        ...
        // Minimal fallback
        if (visibility === 'public' && window.ShareProfile) {
             window.ShareProfile.share(profileId, { suppressSuccessModal: true }).then((res) => {
               const url = res?.accessUrl || res?.publicUrl || `${window.location.origin}/u/${publicHandle}`;
...
window.handleDashboardPublishToggle = async function (event) {
  const el = event.target;
  if (!(el instanceof HTMLInputElement) || el.type !== 'checkbox') return;
  const profileId = el.getAttribute('data-profile-id');
  if (!profileId) return;

  const wantPublic = el.checked;
  const refresh = window.__renderDashboardStatsAndInterviews;
  const cell = el.closest('.dashboard-publish-cell');
  const spinner = cell && cell.querySelector('.dashboard-publish-loading');
  ...
  function setPublishLoading(on) {
    ...
    if (cell) {
      cell.classList.toggle('cursor-wait', on);
    }
    if (on) {
  ...
  try {
    if (wantPublic) {
      ...
      try {
        const result = await window.ShareProfile.share(profileId, { suppressSuccessModal: true });
        if (!result) {
          el.checked = false;
          return;
        }
      ...
      const r = await fetch(`/api/profiles/${encodeURIComponent(profileId)}/unpublish`, {
        method: 'POST',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' }
      });
    ...
    const dash = await fetch('/api/dashboard', { credentials: 'include' });
    if (dash.ok) {
