...
async function loadPDFResume(resumeFileId, profileId) {
  try {
    ...
    if (file.url && file.url.includes('cloudinary.com')) {
      ...
      // Cloudinary raw files have URLs like: https://res.cloudinary.com/cloud/raw/upload/...
      if (file.url.includes('/raw/upload/') || file.mime === 'application/pdf') {
        console.log('[owner-preview] Detected Cloudinary raw file (PDF), using Google Docs Viewer');
    ...
    if (isPDF) {
      if (useGoogleDocsViewer) {
        // For Cloudinary raw files, use Google Docs Viewer with loading state
        resumeSection.innerHTML = `
          ...
          <div class="border border-subtle-light dark:border-subtle-dark rounded-lg overflow-hidden">
            <div class="relative">
              <!-- Loading state -->
              <div id="resume-loading" class="w-full h-[800px] bg-white flex items-center justify-center">
              ...
              <!-- Resume iframe (initially hidden) -->
              <iframe 
                id="resume-iframe"
                src="https://docs.google.com/viewer?url=${encodeURIComponent(file.url)}&embedded=true" 
                class="w-full h-[800px] bg-white hidden"
                title="Resume PDF (Google Docs Viewer)"
                onload="handleResumeLoad()">
              </iframe>
              
              <!-- Error state (initially hidden) -->
              <div id="resume-error" class="w-full h-[800px] bg-red-50 dark:bg-red-900/20 border-2 border-red-200 dark:border-red-800 rounded-lg hidden items-center justify-center">
                <div class="text-center space-y-4">
                  <span class="material-symbols-outlined text-6xl text-red-500">error</span>
                  <div class="space-y-2">
                    <p class="text-lg font-medium text-red-700 dark:text-red-300">Failed to Load Resume</p>
                    <p class="text-sm text-red-600 dark:text-red-400">The PDF viewer couldn't load your resume</p>
                    <button id="retry-resume" class="mt-4 flex items-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg text-sm font-medium mx-auto">
...
function wireButtons(profile) {
  ...
  if (editBtn) {
    editBtn.addEventListener('click', (e) => {
      ...
      window.location.href = `/profile_edit.html?id=${profile.id || profile._id}`;
      
      // Re-enable in case the user navigates back (bfcache handling is separate, but this helps if navigation is cancelled/fails)
      setTimeout(() => {
         btn.disabled = false;
  ...
  if (shareBtn && window.ShareModalController) {
    // Initialize the controller (attaches event listeners to modal buttons)
    window.ShareModalController.init();
    
    shareBtn.addEventListener('click', () => {
      console.log('[owner-preview] Opening share modal for profile:', profile.id);
