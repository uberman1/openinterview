...
(() => {
  ...
  async function ensureSession(){
    try{
      const me=await fetch('/auth/me',{credentials:'include'}); if(me.ok){ const j=await me.json(); if(j?.user) localStorage.setItem('user', JSON.stringify(j.user)); }
    }catch{}
    try{
      const mine=await fetch('/api/profiles/mine',{credentials:'include'}); if(mine.ok){ const p=await mine.json(); if(p?.id) localStorage.setItem('profile', JSON.stringify(p)); }
    }catch{}
    try{
      const u=getUser();
      if(u&&u.id){
        const s=await fetch(`/api/profiles/status/${u.id}`);
        if(s.ok){
          const d=await s.json();
          if(d.storage?.maxResumeFileSizeBytes) API.maxResumeMB=d.storage.maxResumeFileSizeBytes/(1024*1024);
        }
      }
    }catch{}
  }
  ...
  const API = {
    list: () => {
      const p=getProfile(); return p?`/api/files?profileId=${p.id}`:null;
    },
    upload: (kind) => {
  ...
  async function triggerUpload(){
    await ensureSession();
    if(!API.upload('resume')){ alert('No profile selected'); return; }
    const input=document.createElement('input'); input.type='file';
    input.accept='.pdf,.doc,.docx,.ppt,.pptx,.key,.pages';
    input.style.display='none'; document.body.appendChild(input);
    const cleanup=()=>{ try{ input.remove(); }catch{} };
    input.addEventListener('change', async ()=>{
      const file=input.files&&input.files[0]; cleanup(); if(!file) return;
  ...
  async function init(){
    if(document.body.dataset.uploadsBound==='true') return;
    document.body.dataset.uploadsBound='true';
    await ensureSession();
    loadTables();
    bindActions();
  }
