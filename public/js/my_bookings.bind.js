...
(async function initMyBookings() {
    ...
    window.openSmsErrorModal = function (errorMessage) {
        const modal = document.getElementById('sms-error-modal');
        const msgEl = document.getElementById('sms-error-message');
        if (!modal || !msgEl) return;

        msgEl.textContent = errorMessage || 'SMS sending failed.';
        modal.classList.remove('hidden');
        modal.classList.add('flex');
    };

    window.closeSmsErrorModal = function () {
        const modal = document.getElementById('sms-error-modal');
        if (!modal) return;
        modal.classList.add('hidden');
        modal.classList.remove('flex');
    ...
    async function updateStatus(id, newStatus, btnElement) {
        ...
        try {
            ...
            // If backend reported SMS failure, show it to the user.
            try {
                const data = await res.json();
                if (data?.sms?.attempted && data.sms.success === false && data.sms.errorMessage) {
                    openSmsErrorModal(`SMS failed: ${data.sms.errorMessage}`);
                }
            } catch (e) {
