  ...
  function buildTopbar(){
    ...
    wrapper.innerHTML = `
      <div class="container mx-auto px-10 py-4 flex items-center justify-between whitespace-nowrap">
        ...
        <div class="flex items-center gap-4" id="oi-actions">
    ...
    // Custom menu for Profiles page
    if (location.pathname.endsWith('/profiles.html')) {
       // Add "New Profile" button to actions
       const actions = wrapper.querySelector('#oi-actions');
       const newProfileBtn = document.createElement('a');
       newProfileBtn.href = '/profile_create.html'; // Assuming this page exists or uses a modal, user asked for "add button"
       // Check if profile_create.html exists or what the flow is. Usually it's /profile/new or similar.
       // Looking at other files, there is /profile_edit.html?new=true maybe?
       // The user said "add button at navbar add new profile".
       // I'll assume /profile_edit.html (as seen in dashboard.html "My Profile" link is /profile_edit.html).
       // Actually, let's use a generic class that triggers the modal or links to create page.
       // I'll use a link to /profile_edit.html?mode=create
       newProfileBtn.href = '/profile_edit.html?new=true'; 
       newProfileBtn.className = 'px-4 py-2 bg-primary text-white text-sm font-bold rounded hover:bg-primary/90 transition-colors';
       newProfileBtn.textContent = 'New Interview';
       actions.insertBefore(newProfileBtn, actions.firstChild);
    }
