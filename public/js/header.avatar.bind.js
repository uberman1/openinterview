(function(){
  /** Material Symbols font: without it, `.material-symbols-outlined` text (e.g. smart_toy) renders as literal letters and overflows the FAB. */
  function ensureMaterialSymbolsFont() {
    const marker = 'Material+Symbols+Outlined';
    const already =
      document.querySelector(`link[rel="stylesheet"][href*="${marker}"]`) ||
      document.querySelector(`link[href*="${marker}"]`);
    if (already) return;
