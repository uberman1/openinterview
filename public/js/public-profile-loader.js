  ...
  if (resumeFileId) {
    try {
      ...
      if (res.ok) {
        ...
        if (isCloudinaryRaw) {
          // Proxy through /api/proxy/pdf so the server sets Content-Disposition: inline.
          // fl_inline doesn't work on /raw/upload/ Cloudinary URLs because raw files
          // bypass the Cloudinary transformation pipeline entirely.
          const proxyUrl = `/api/proxy/pdf?url=${encodeURIComponent(file.url)}`;
          resumeContainer.innerHTML = `
            <div class="block md:hidden p-6 rounded-xl border border-subtle-light dark:border-subtle-dark bg-white dark:bg-subtle-dark text-center">
              <p class="text-sm font-medium break-words max-w-full mb-4">${file.name || 'Resume PDF'}</p>
              <div class="flex flex-col gap-3 items-center">
                <a href="${proxyUrl}" target="_blank" rel="noopener noreferrer"
                   class="w-full max-w-xs px-4 py-2 bg-primary text-white rounded-lg text-sm font-medium text-center hover:bg-primary/90 transition-colors">
                  Open Resume
                </a>
                <a href="${file.url}"
                   onclick="return downloadRawFile(event, '${file.url}', '${file.name}')"
                   target="_blank" rel="noopener noreferrer"
                   class="w-full max-w-xs px-4 py-2 border border-primary text-primary rounded-lg text-sm font-medium text-center hover:bg-primary/10 transition-colors">
                  Download Resume
                </a>
              </div>
              <p class="text-xs text-muted-light dark:text-muted-dark mt-3">Mobile browsers may open PDFs in a separate viewer.</p>
            </div>
            <div class="hidden md:block relative bg-white" id="resume-wrapper">
            ...
            <div class="p-4 bg-background-light dark:bg-subtle-dark hidden md:flex flex-col items-center gap-3 lg:flex-row lg:justify-between border-t border-subtle-light dark:border-subtle-dark">
              <p class="text-sm font-medium break-words max-w-full text-center lg:text-left">${file.name}</p>
              <a 
                href="${file.url}" 
                onclick="return downloadRawFile(event, '${file.url}', '${file.name}')"
                target="_blank"
                rel="noopener noreferrer"
                class="flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors text-sm font-medium shrink-0">
                Download Resume
              </a>
            </div>
          `;
