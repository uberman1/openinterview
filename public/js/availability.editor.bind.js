...
(function initAvailabilityEditor() {
  ...
  function render() {
    ...
    // Window is fixed 60 days
    const winInput = root.querySelector('#window');
    if (winInput) {
      winInput.value = '60 (Fixed)';
      winInput.disabled = true;
    }
    
    // Reuse #increments as Duration selector
    const incSelect = root.querySelector('#increments');
    if (incSelect) {
       // Update label if possible
       const label = root.querySelector('label[for="increments"]');
       if (label) label.textContent = 'Meeting Duration';
       
       // Update options if needed (assuming they are 15, 30, 60 etc)
       // Just set value
       incSelect.value = model.rules.durationMinutes;
    }
  ...
  root.querySelector('#increments')?.addEventListener('change', (e) => {
    ...
    model = setRules(model, { durationMinutes: minutes });
  });
