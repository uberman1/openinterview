...
async function autoInitializeSchema(client) {
  try {
    ...
    // Widget magic-login tokens (one-time email login)
    await applyMagicLoginTokensMigration(client);

    // Billing: pending downgrade + cancel-at-period-end metadata
    await applyEntitlementsBillingColumnsMigration(client);

    // Oi Business beta waitlist
    await applyOiBusinessWaitlistMigration(client);
...
/** Public profiles oldest-first (FIFO unpublish on downgrade / cancel). */
export async function listPublicProfilesByUserOldestFirst(userId) {
  const { rows } = await pool.query(
    `SELECT * FROM profiles WHERE user_id = $1 AND visibility = 'public' ORDER BY created_at ASC`,
    [userId]
  );
  return rows.map(formatProfile);
}

/** Count published profiles (source of truth for shares_used reconciliation). */
export async function countPublicProfilesByUser(userId) {
  const { rows } = await pool.query(
    `SELECT COUNT(*)::int AS count FROM profiles WHERE user_id = $1 AND visibility = 'public'`,
    [userId]
  );
  return Number(rows[0]?.count ?? 0);
}

export async function setSharesUsed(userId, sharesUsed) {
  const n = Math.max(0, Number(sharesUsed) || 0);
  const { rows } = await pool.query(
    `UPDATE entitlements SET shares_used = $1, updated_at = NOW() WHERE user_id = $2 RETURNING *`,
    [n, userId]
  );
...
async function applyMagicLoginTokensMigration(client) {
  try {
    const { rows } = await client.query(`
      SELECT EXISTS (
        SELECT FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'magic_login_tokens'
      ) AS exists;
    `);
    if (!rows[0]?.exists) {
      console.log('[pg] 🔄 Applying magic_login_tokens migration...');
      const { readFileSync } = await import('fs');
      const { fileURLToPath } = await import('url');
      const { dirname, join } = await import('path');
      const __filename = fileURLToPath(import.meta.url);
      const __dirname = dirname(__filename);
      const migrationPath = join(__dirname, 'migrations', 'add-magic-login-tokens.sql');
      const migration = readFileSync(migrationPath, 'utf8');
      await client.query(migration);
      console.log('[pg] ✅ magic_login_tokens table ready');
    } else {
      console.log('[pg] ✅ magic_login_tokens table exists');
    }
  } catch (error) {
    console.error('[pg] ⚠️  magic_login_tokens migration failed:', error.message);
  }
}

async function applyOiBusinessWaitlistMigration(client) {
  try {
    await client.query(`
      CREATE TABLE IF NOT EXISTS oi_business_waitlist (
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
        email TEXT NOT NULL UNIQUE,
        domain TEXT NOT NULL,
        source TEXT NOT NULL DEFAULT 'oi-business',
        user_agent TEXT,
        ip_address TEXT,
    ...
    `);
    await client.query(`
      CREATE INDEX IF NOT EXISTS idx_oi_business_waitlist_created_at
      ON oi_business_waitlist (created_at DESC)
    `);
    await client.query(`
      CREATE INDEX IF NOT EXISTS idx_oi_business_waitlist_domain
      ON oi_business_waitlist (domain)
    `);
    console.log('[pg] ✅ oi_business_waitlist table ready');
  } catch (error) {
    console.error('[pg] ⚠️  oi_business_waitlist migration failed:', error.message);
  }
}

async function applyEntitlementsBillingColumnsMigration(client) {
  try {
    await client.query(`
    ...
    `);
    console.log('[pg] ✅ entitlements billing columns (pending_downgrade_plan, stripe_cancel_at_period_end)');
  } catch (error) {
    console.error('[pg] ⚠️  entitlements billing columns migration failed:', error.message);
  }
}
  ...
  try {
    ...
    for (let i = 0; i < maxAttempts; i++) {
      ...
      try {
        const { rows } = await client.query(
          `UPDATE profiles
           SET public_handle = $1, visibility = 'public', updated_at = NOW()
           WHERE id = $2 AND user_id = $3
           RETURNING *`,
          [candidate, profileId, userId]
        );
        if (!rows.length) {
          throw new Error('Profile not found or access denied');
        }
...
export async function getPlanByCode(code) {
  if (!code) return null;
  const { rows } = await pool.query(
    'SELECT * FROM plans WHERE LOWER(code) = LOWER($1) AND is_active = true',
    [code]
  );
...
export async function updateEntitlement(userId, updates) {
  ...
  const fieldMap = {
    ...
    stripeSubscriptionId: 'stripe_subscription_id',
    stripeSubscriptionStatus: 'stripe_subscription_status',
    pendingDowngradePlan: 'pending_downgrade_plan',
    stripeCancelAtPeriodEnd: 'stripe_cancel_at_period_end'
  };
...
export async function resetMonthlyUsage(userId, resetDate) {
  ...
  try {
    ...
    await client.query(`
      UPDATE entitlements 
      SET bookings_used = 0, views_used = 0, credits_reset_at = $1, updated_at = NOW()
      WHERE user_id = $2
    `, [resetDate, userId]);
    ...
    try {
      const { purgeMediaOverageIfNeeded } = await import('../services/plan-compliance.js');
      await purgeMediaOverageIfNeeded(userId);
    } catch (purgeErr) {
      console.error('[pg] purgeMediaOverageIfNeeded after resetMonthlyUsage:', purgeErr?.message || purgeErr);
    }

    try {
      const { reconcileSharesUsed } = await import('../services/share-credits.js');
      await reconcileSharesUsed(userId);
    } catch (reconcileErr) {
      console.error('[pg] reconcileSharesUsed after resetMonthlyUsage:', reconcileErr?.message || reconcileErr);
    }
  } catch (error) {
    await client.query('ROLLBACK');
    throw error;
  ...
  try {
    ...
    if (now >= resetDate) {
      ...
      try {
        ...
        console.error('[pg] purgeMediaOverageIfNeeded after lazy reset:', purgeErr?.message || purgeErr);
      }
      ...
      try {
        ...
        console.error('[pg] reconcileSharesUsed after lazy reset:', reconcileErr?.message || reconcileErr);
      }

      return (await getEntitlement(userId)) || updatedEntitlement;
    }
...
function formatEntitlement(row) {
  ...
  return {
    ...
    pendingDowngradePlan: row.pending_downgrade_plan || null,
    stripeCancelAtPeriodEnd: row.stripe_cancel_at_period_end || null,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
...
export default {
  ...
  listProfilesByUser,
  listPublicProfilesByUserOldestFirst,
  countPublicProfilesByUser,
  setSharesUsed,
  createProfile,
  updateProfile,
