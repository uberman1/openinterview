...
export function mountAdminConsoleRoutes(app: Express, base: string) {
  app.get(`${base}/admin/users`, requireAdmin as any, async (_req, res) => {
    const users = await listUsers();
    res.json({ users });
  });
  
  app.get(`${base}/admin/flags`, requireAdmin as any, async (_req, res) => {
    const flags = await getFlags();
    res.json({ flags });
  });
  
  app.patch(`${base}/admin/flags`, requireAdmin as any, async (req: Request, res) => {
    const patch = req.body || {};
    const flags = await updateFlags(patch);
    res.json({ flags });
  });
}
