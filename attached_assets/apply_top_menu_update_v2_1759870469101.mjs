...
cp('public/js/topmenu.unify.v2.bind.js','public/js/topmenu.unify.v2.bind.js');
...
function injectBinder(route, fileName){
  const marker = `/* TOPMENU_UNIFY_V2_${route.replace(/[\/\.]/g,'_')} */`;
...
app.get('${route}', (req,res) => {
  try{
    ...
    if (!html.includes('/js/topmenu.unify.v2.bind.js')){
      html = html.replace('</body>', '<script src="/js/topmenu.unify.v2.bind.js" defer></script></body>');
    }
...
ROUTES.forEach(r => injectBinder(r, r.slice(1)));
src = ensureImports(src);
fs.writeFileSync(idx, src, 'utf8');
console.log('✔ Patched index.js with Top Menu v2 binder on target routes');
