...
def run_security(base_url, contract, outdir):
    ...
    with sync_playwright() as pw:
        ...
        csp_header = "content-security-policy" in headers; csp_meta = re.search(r'Content-Security-Policy', html, re.I) is not None
        if contract.get("headers",{}).get("expect_csp", False) and not (csp_header or (allow_meta and csp_meta)): results["issues"].append("CSP not found in headers or meta")
        if results["issues"]: results["status"]="FAIL"
        context.close(); browser.close()

    with open(os.path.join(outdir,"security.json"),"w",encoding="utf-8") as f: json.dump(results,f,indent=2)
    with open(os.path.join(outdir,"security.txt"),"w",encoding="utf-8") as f: f.write(json.dumps(results,indent=2))
    return results