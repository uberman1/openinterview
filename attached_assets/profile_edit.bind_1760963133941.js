// profile_edit.bind.js — binder for the editable profile page
// Non-destructive: works with existing markup; optional sections are guarded.
// Requires helpers in app.js and data-store.js
import { $, $$, toast } from './app.js';
import { getProfile, updateProfile, publishProfile } from './data-store.js';
...
(function initProfileEditBinder(){
  ...
  const hasEnhancedRules = !!window.__OI_ENHANCED_RULES__;
  if (hasEnhancedRules) initEnhancedRulesUI(profile);

  // Global Save (if present)
  ...
  function currentModel(){
    ...
    const bioEl = $('textarea[placeholder^="Tell us"]');
    if (nameEl)  model.name = nameEl.value.trim();
    if (titleEl) model.title = titleEl.value.trim();
    if (locEl)   model.location = locEl.value.trim();
    if (bioEl)   model.bio = bioEl.value.trim();

    // Contact inline (if present)
    const phoneEl = $('input[placeholder="e.g. +1 234 567 890"]');
    const emailEl = $('input[placeholder="your.email@example.com"]');
    if (phoneEl) model.phone = phoneEl.value.trim();
    if (emailEl) model.email = emailEl.value.trim();
    ...
    model.links = linkRows.map(row => {
      ...
      const kind = label.includes('linkedin') ? 'linkedin' :
                   label.includes('portfolio') ? 'portfolio' : 'other';
      return { kind, url: input?.value?.trim()||'' };
    });

    // Highlights
    const hi = $('section:has(h2:contains("Highlights")) textarea');
    if (hi) model.highlights = hi.value;

    // Availability
    model.availability = readAvailabilityFromUI(model.availability);

    // Profile name
    const pn = document.getElementById('profileNameInput');
    if (pn) model.profileName = pn.value.trim();
  ...
  function hydrate(p){
    // Basics
    $('input[placeholder="Your Name"]')?.setAttribute('value', p.name||'');
    $('input[placeholder="Your Title"]')?.setAttribute('value', p.title||'');
    $('input[placeholder="City, Country"]')?.setAttribute('value', p.location||'');
    const bioEl = $('textarea[placeholder^="Tell us"]');
    ...
    // Profile name bar
    const pn = document.getElementById('profileNameInput');
    if (pn) pn.value = p.profileName || '';

    // Availability
    paintAvailability(p.availability);
  }

  function wireBasics(p){ /* no-op placeholders */ }
    ...
    uploadBtn?.addEventListener('click', async ()=>{
      ...
      for (const f of files){
        ...
        if (isResumeLike && !p.resume?.url){
          ...
          try { await analyzeResumeAndPopulate(f, url, p); didAnalyze = true; }
  ...
  function renderResumeAndAttachments(p){
    const container = $$('section:has(h2:contains("Resume")) .space-y-4')[0] || $$('section:has(h2:contains("Resume & Attachments")) .space-y-4')[0];
    if (!container) return;
    container.innerHTML = '';
  ...
  // ---------- Resume import (new top-of-page selector variants are supported by existing logic) ----------
  function wireResumeImportUI(p){
    // Keep compatibility with earlier markup
    ...
    function paintOptions(){
      ...
      const mk = (v, label, extra={})=>{
        ...
        Object.entries(extra).forEach(([k,val])=> opt.setAttribute(k, val));
        return opt;
      };
  ...
  function wireAvailability(p){
    ...
    // Add Block
    addBlockButtons.forEach((_btn, idx)=>{
      _btn.addEventListener('click', ()=>{
        const model = readAvailabilityFromUI(p.availability);
        const day = [1,2,4][idx] ?? 1; // based on sample markup
  ...
  function initEnhancedRulesUI(p){
    const sel = $('#min-notice-select');
    const custom = $('#min-notice-custom');
    if (sel) {
      const toggleCustom = ()=>{
        if (sel.value === 'custom') { custom?.classList.remove('hidden'); custom?.focus(); }
    ...
    // Duration chips
    const chips = $$('#durations-chips [data-duration]');
    if (chips.length){
      chips.forEach(ch=>{
        ch.style.cursor = 'pointer';
        ch.addEventListener('click', ()=>{
          chips.forEach(x=> x.classList.remove('ring','ring-primary'));
          ch.classList.add('ring','ring-primary');
          const dur = parseInt(ch.getAttribute('data-duration')||'30',10);
          p.availability = p.availability || {};
          p.availability.durationMins = dur;
        });
      });
    }
  }
  ...
  function paintAvailability(model){
    ...
    const dayDefs = [
      { id:'sun', idx:0 },{ id:'mon', idx:1 },{ id:'tue', idx:2 },
      { id:'wed', idx:3 },{ id:'thu', idx:4 },{ id:'fri', idx:5 },{ id:'sat', idx:6 },
    ];
    ...
    // Enhanced Minimum Notice
    const sel = $('#min-notice-select');
    const custom = $('#min-notice-custom');
    if (sel) {
      const h = typeof model.minNoticeHours === 'number' ? model.minNoticeHours : 24;
      if (h===0) sel.value = '0';
      else if ([4,12,24].includes(h)) sel.value = String(h);
      else { sel.value = 'custom'; if (custom) { custom.classList.remove('hidden'); custom.value = String(h); } }
    }
    ...
    if (rulesBox){
      ...
      if (cap) cap.value = (model.dailyCap ?? '');
    }
  }
  ...
  function readAvailabilityFromUI(fallback){
    ...
    if (sel) {
      if (sel.value === 'custom') {
        const hv = parseInt((custom?.value||'').trim(), 10);
        model.minNoticeHours = Number.isFinite(hv) ? hv : 24;
      } else {
        const v = parseInt(sel.value, 10);
        model.minNoticeHours = Number.isFinite(v) ? v : 24;
      }
    }

    // Buffers
    const rulesBox = $$('section:has(h2:contains("Set Your Availability")) .p-6.border.rounded-lg')[1];
    const numInputs = rulesBox?.querySelectorAll('input[type="number"]');
    ...
    // Daily Cap: blank = unlimited (undefined)
    const capEl = rulesBox?.querySelector('#daily-cap');
    if (capEl) {
      const v = (capEl.value||'').trim();
      model.dailyCap = v==='' ? undefined : parseInt(v,10);
      if (!Number.isFinite(model.dailyCap)) model.dailyCap = undefined;
    }

    return model;
  }

  // Booking slots generator with daily cap
  ...
  function buildSlotsForDate(date, model){
    ...
    if (typeof model.dailyCap === 'number' && Number.isFinite(model.dailyCap)) {
      return slots.slice(0, model.dailyCap);
    }
    return slots;
  }

  // Top bars: Edit/Publish and Profile Name
  function wireTopBars(p){
    // Edit
    const editBtn = document.querySelector('[data-action="edit-profile"]');
    editBtn?.addEventListener('click', ()=>{
      const id = p.id || '';
      location.href = `/profile_edit_enhanced.html${id ? ('?id='+encodeURIComponent(id)) : ''}`;
    });
    // Publish
    const publishBtn = document.querySelector('[data-action="publish-profile"]');
    publishBtn?.addEventListener('click', async ()=>{
      try {
        const model = currentModel();
        await updateProfile(model);
        if (typeof publishProfile === 'function') {
          const res = await publishProfile(model.id);
          toast('Profile published' + (res?.publicUrl ? `: ${res.publicUrl}` : ''));
        } else {
          model.status = 'live';
          model.publicSlug = model.publicSlug || (model.id||'').slice(0,8);
          model.publicUrl = model.publicUrl || (`${location.origin}/p/${model.publicSlug}`);
          await updateProfile(model);
          toast('Profile published: ' + model.publicUrl);
        }
      } catch(e){ console.error(e); toast('Publish failed', {type:'error'}); }
    });
    // Profile name save
    const nameInput = document.getElementById('profileNameInput');
    const saveNameBtn = document.getElementById('saveProfileNameBtn');
    if (nameInput) nameInput.value = p.profileName || '';
    saveNameBtn?.addEventListener('click', async ()=>{
      try {
        p.profileName = (nameInput?.value||'').trim();
        await updateProfile(p);
        window.dispatchEvent(new CustomEvent('profile:named', { detail:{ id:p.id, name:p.profileName }}));
        toast('Profile name saved');
      } catch(e){ console.error(e); toast('Failed to save name', {type:'error'}); }
    });
  }
  ...
  async function fakeUpload(file){
    // TODO: replace with server upload; returns temp blob URL
    return URL.createObjectURL(file);
  }
})();

// AI resume analysis stub — expects backend route; throws if not available
async function analyzeResumeAndPopulate(file, uploadedUrl, p){
  ...
  const ensureRow = (label)=>{
    ...
    let row = Array.from(container.querySelectorAll('p')).find(pp=> pp.textContent.trim().toLowerCase()===label)?.closest('div.flex.items-end.gap-2');
    if (!row){
  ...
  socialMap.forEach(({kind,url})=>{
    ...
    row?.querySelector('input')?.setAttribute('value', url);
  });
