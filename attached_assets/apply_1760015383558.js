/**
 * OpenInterview v4.1 Mover & Versions Updater
 * - Moves profile_v4_1_package under /public
 * - Updates public/data/versions.json
...
function updateVersionsJson() {
  ...
  if (fs.existsSync(versionsJsonPath)) {
    try {
      ...
      warn('versions.json parse failed; will overwrite minimally.');
    }
  }
  ...
  if (Array.isArray(obj)) {
    const idx = obj.findIndex(x => x && x.version === '4.1');
    if (idx >= 0) obj[idx] = entry;
    else obj.push(entry);
    out = obj;
  } else if (obj && typeof obj === 'object') {
    // If it looks like a single object, keep simple fields.
    out = { ...(obj || {}), ...entry };
  } else {
    out = [entry];
  }

  fs.writeFileSync(versionsJsonPath, JSON.stringify(out, null, 2));
...
(async function main() {
  ...
  log('• versions.json updated to point to static page.');
  if (testCode === 0) {
    log('• Tests: PASSED ✅');
  } else {
  ...
  log('\nNext steps:');
  log('1) Restart your main server (Stop → Run).');
  log('2) Open /profile_v4_1_package/public/index.html or use your version picker.');
  log('3) Verify: attachments download, resume pagination, calendar .ics.');
})();
