...
def run(page_html_path, response_headers, contract, artifact_dir):
    dom = contract.get("dom",{}); headers = contract.get("headers",{})
    results = {"dom": {"failures": []}, "headers": {"failures": []}, "status":"PASS"}
    html = ""
    try:
        with open(page_html_path,"r",encoding="utf-8") as f: html = f.read()
    except Exception as e:
        results["dom"]["failures"].append(f"READ_ERROR: {e}")

    if html: results["dom"]["failures"].extend(check_dom_selectors(html, dom.get("must_exist",[])))

    expect_csp = headers.get("expect_csp", False); allow_meta = headers.get("allow_meta_csp", True)
    ...
    if results["dom"]["failures"] or results["headers"]["failures"]: results["status"]="FAIL"

    os.makedirs(artifact_dir, exist_ok=True)
    with open(os.path.join(artifact_dir,"contract.json"),"w",encoding="utf-8") as f: json.dump(results,f,indent=2)
    with open(os.path.join(artifact_dir,"contract.txt"),"w",encoding="utf-8") as f: f.write(json.dumps(results,indent=2))
    return results