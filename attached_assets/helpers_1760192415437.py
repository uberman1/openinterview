...
def snapshot_source(src_path, out_path):
    try:
...
def diff_images(imgA_bytes, imgB_bytes):
    ...
    if imgA.size != imgB.size: return 1.0, None
    diff = ImageChops.difference(imgA, imgB)
    if not diff.getbbox(): return 0.0, None
    total = imgA.size[0]*imgA.size[1]
    nonzero = sum(1 for px in diff.convert("L").getdata() if px != 0)
