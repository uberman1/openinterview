// /server/auth.routes.js — m04a: real signup returns 201; unified router
...
export function mountAuth(app, base){
  ...
  // attach user using real adapter
  ...
  router.post('/auth/signup', async (req, res)=>{
    try{
      ...
      return res.status(code).json({ error: { code: e.code || 'BAD_INPUT', message: e.message || 'Invalid input' } })
    }
  })
  ...
  router.post('/auth/login', async (req, res)=>{
    try{
      ...
      return res.status(401).json({ error: { code: e.code || 'INVALID_CREDENTIALS', message: e.message || 'Invalid credentials' } })
    }
  })
