// OpenInterview • Availability (Calendly-style) — integrated client logic
...
function renderWeekly(){
  $$('.blocks').forEach(el=>{
    ...
    el.innerHTML = '';
    (state.weekly[day]||[]).forEach((blk,i)=>{
      const row = document.createElement('div');
      row.className='flex items-center gap-2';
      row.innerHTML = `
        <input type="time" value="${blk[0]}" class="rounded-lg flex-1 start">
        <span class="text-muted-light">–</span>
        <input type="time" value="${blk[1]}" class="rounded-lg flex-1 end">
      ...
      row.querySelector('.remove').addEventListener('click',()=>{ state.weekly[day].splice(i,1); renderWeekly(); });
      el.appendChild(row);
    });
  });
}
...
function bindWeekly(){
  $$('.add-block').forEach(btn=>{
    btn.addEventListener('click',()=>{
      ...
      state.weekly[d].push(['09:00','12:00']);
      renderWeekly();
    });
  });
