...
export default function ShareMock() {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 720, margin: "0 auto" }}>
      ...
      {!shares[token] && (
        <div style={{ color: "#dc2626" }}>Invalid or unknown token.</div>
      )}
      {shares[token] && !video && (
        <div style={{ color: "#f59e0b" }}>Item exists but not found locally. This is expected if the share was generated on another device.</div>
      )}
      {video && (
        <div style={{ border: "1px solid #e5e7eb", borderRadius: 12, padding: 16, background: "#fafafa" }}>
          <div style={{ fontWeight: 600, marginBottom: 8 }}>{video.name}</div>
          <div style={{ fontSize: 12, color: "#6b7280" }}>{Math.round(video.size/1024)} KB • {new Date(video.ts).toLocaleString()}</div>
          <div style={{ marginTop: 12, padding: 24, textAlign: "center", border: "1px dashed #cbd5e1", borderRadius: 8 }}>
            Mock player preview
          </div>
        </div>
      )}
