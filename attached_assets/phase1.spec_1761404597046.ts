...
test.describe('Phase 1 — HTML & UX integrity', () => {
  ...
  test('Only "Save & Return" is present', async ({ page }) => {
    ...
    await expect(page.locator('button', { hasText: /^Save Profile$/i })).toHaveCount(0);
  ...
  test('Availability section has help text, no local Save/Revert', async ({ page }) => {
    ...
    await expect(avail.locator('button', { hasText: /^Save$/i })).toHaveCount(0);
    await expect(avail.locator('button', { hasText: /^Revert$/i })).toHaveCount(0);
