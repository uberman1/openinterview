...
const MODULE = (process.env.MODULE || process.argv[2] || 'module4').toLowerCase()
...
async function runAll() {
  const files = fs.readdirSync(path.join(process.cwd(), 'tests')).filter(f => f.startsWith('module') and f.endswith('.json'))
  files.sort((a, b) => {
