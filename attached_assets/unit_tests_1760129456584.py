...
class TestPasswordPage(unittest.TestCase):
    ...
    def test_labels(self): self.assertRegex(self.html, r'<label[^>]*for=["\']email["\']'); self.assertRegex(self.html, r'<label[^>]*for=["\']new_password["\']'); self.assertRegex(self.html, r'<label[^>]*for=["\']confirm_password["\']')
