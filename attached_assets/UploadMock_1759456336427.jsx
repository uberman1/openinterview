  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <ul style={{ marginTop: 16, padding: 0, listStyle: "none", display: "grid", gap: 8 }}>
        {items.length === 0 && <li style={{ color: "#64748b" }}>No uploads yet.</li>}
        {items.map(it => (
          <li key={it.id} style={{ border: "1px solid #e5e7eb", borderRadius: 8, padding: 12, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <div style={{ fontWeight: 600 }}>{it.name}</div>
              <div style={{ fontSize: 12, color: "#6b7280" }}>{Math.round(it.size/1024)} KB • {new Date(it.ts).toLocaleString()}</div>
            </div>
            <button onClick={() => genShare(it.id)} style={{ padding: "6px 10px", borderRadius: 8, border: "1px solid #e5e7eb", background: "#f8fafc", cursor: "pointer" }}>
              Generate Share Link
            </button>
          </li>
        ))}
