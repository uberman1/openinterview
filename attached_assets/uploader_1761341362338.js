...
export async function uploadVideoToCloudinary(file, profileId) {
  const sig = await getSignature({ profileId, folder: 'openinterview/videos', resource_type: 'auto' });
  ...
  const res = await fetch(cldUploadEndpoint(cloudName, 'auto'), { method: 'POST', body: fd }).then(r => r.json());
  ...
  return {
    ...
    // replace default thumbnail on first video upload with auto poster
    posterMode: 'auto',
...
export async function uploadImageToCloudinary(file, profileId) {
  const sig = await getSignature({ profileId, folder: 'openinterview/thumbnails', resource_type: 'image' });
  const { cloudName, apiKey, timestamp, signature, uploadPreset, folder } = sig;
  ...
  const res = await fetch(cldUploadEndpoint(cloudName, 'image'), { method: 'POST', body: fd }).then(r => r.json());
  if (res.error) throw new Error(res.error.message);
