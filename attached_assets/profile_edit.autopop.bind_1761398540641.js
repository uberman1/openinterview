...
import { getProfile, updateProfile, listAssets, createAsset } from '/js/data-store.js';
...
(function initAutoPopulate() {
  ...
  async function hydrateDropdown() {
    try {
      const assets = await listAssets?.({ type: 'resume', profileId }) ?? [];
  ...
  async function applyParsedData(assetId, data) {
    try {
      ...
      await updateProfile?.(profileId, next);
