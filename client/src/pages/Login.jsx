...
export default function Login(){
  ...
  async function onSubmit(e){
    ...
    try {
      if (devMode) {
        const fakeToken = `dev_${email}_${Date.now()}`;
        setToken(fakeToken);
      ...
      <form onSubmit={onSubmit} style={styles.card}>
        ...
        <div style={styles.toggleWrap}>
          <label style={styles.toggleLabel}>
            ...
            />
            <span style={styles.toggleText}>Dev mode</span>
          </label>
          {devMode && <span style={styles.devBadge}>Any credentials work</span>}
        </div>
...
const styles = {
  ...
  toggleWrap: { marginBottom:'1rem', display:'flex', alignItems:'center', gap:'0.5rem' },
  toggleLabel: { display:'flex', alignItems:'center', gap:'0.5rem', cursor:'pointer' },
  checkbox: { width:16, height:16, cursor:'pointer' },
  toggleText: { fontSize:'0.9rem', color:'#d1d5db' },
  devBadge: { fontSize:'0.75rem', padding:'0.25rem 0.5rem', background:'#065f46', color:'#6ee7b7', borderRadius:4, fontWeight:600 },
