...
// Rate limit all profile routes
app.use('/api/profiles', profileRateLimiter);
...
app.patch("/api/profiles/:id/default", requireAuth, async (req, res) => {
  try {
...
app.post("/api/profiles/:id/attachments", async (req, res) => {
  try {
    const profile = await pgClient.getProfile(req.params.id);
    if (!profile) return res.status(404).json({ error: 'Profile not found' });
...
app.delete("/api/profiles/:id/attachments/:fileId", async (req, res) => {
  try {
    const profile = await pgClient.getProfile(req.params.id);
    if (!profile) return res.status(404).json({ error: 'Profile not found' });
    // Attachments handling deferred to later milestones
...
app.patch("/api/profiles/:id/resume", async (req, res) => {
  try {
    const profile = await pgClient.getProfile(req.params.id);
    if (!profile) return res.status(404).json({ error: 'Profile not found' });
...
// Public profile assembly - WP1: Uses pgClient (Postgres)
app.get("/api/public/profile/:handle", async (req, res) => {
  try {
    const handleOrId = req.params.handle;

    // Try to get profile by handle first, then by ID if not found
    let profile = await pgClient.getProfileByHandle(handleOrId);
    if (!profile) {
      // If not found by handle, try by profile ID
      profile = await pgClient.getProfile(handleOrId);
    }
    ...
    if (profile.resume_file_id) {
      resume = await pgClient.getFile(profile.resume_file_id);
    }

    // Attachments deferred to later milestones
    const attachments = [];

    res.json({ ...profile, resume, attachments });
  } catch (e) {
...
app.post("/api/profiles/:id/share", requireAuth, async (req, res) => {
  try {
    const { id } = req.params;
    const userId = req.user.id;
    ...
    if (profile.visibility === 'public') {
      ...
      if (!publicHandle) {
        ...
        publicHandle = `${baseName}-${profile.id.slice(0, 6)}`
          .toLowerCase()
          .replace(/[^a-z0-9-]/g, '-')
          .replace(/-+/g, '-')
          .replace(/^-|-$/g, '');

        await pgClient.updateProfile(id, { publicHandle });
      }
      ...
      return res.json({
        ...
        url: `${baseUrl}/u/${publicHandle}`,      // Compatibility
        sharesUsed,
        sharesLimit,
    ...
    // Increment share usage (persist)
    await pgClient.updateEntitlement(userId, { sharesUsed: sharesUsed + 1 });
    ...
    const updatedProfile = await pgClient.updateProfile(id, {
      publicHandle,
      visibility: 'public',
      shareCount: (profile.shareCount || 0) + 1
    });

    console.log(`[share] Profile ${id} shared by user ${userId}. Shares used: ${entitlement.sharesUsed}/${sharesLimit}`);

    const baseUrl = process.env.BASE_URL || `${req.protocol}://${req.get('host')}`;
    const ent = await pgClient.getEntitlement(userId);
    res.json({
      success: true,
      publicUrl: `${baseUrl}/u/${updatedProfile.publicHandle}`,
      sharesUsed: ent?.sharesUsed ?? sharesUsed + 1,
      sharesLimit,
      sharesRemaining: sharesLimit === null ? 'Unlimited' : Math.max(0, effectiveLimit - (ent?.sharesUsed ?? sharesUsed + 1))
    });
  } catch (error) {
...
app.post("/api/profiles/:id/invite", requireAuth, async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const baseUrl = process.env.BASE_URL || `${req.protocol}://${req.get('host')}`;
    const profileUrl = `${baseUrl}/u/${profile.publicHandle || profile.id}`;
...
app.get("/api/dashboard", requireAuth, async (req, res) => {
  try {
    ...
    if (profile && profile.publicHandle) {
      const baseUrl = process.env.BASE_URL || `${req.protocol}://${req.get('host')}`;
      shareLink = {
        url: `${baseUrl}/u/${profile.publicHandle}`,
