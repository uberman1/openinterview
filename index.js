...
import { anonUserIdSetOptions } from './server/config/anonUserIdCookie.js';
...
import { uploadRateLimiter, aiParseRateLimiter, profileRateLimiter, aiChatRateLimiter } from './server/middleware/rateLimiter.js';

// WP9: Credits management
import { getCreditsInfo, getUpgradedCredits } from './server/services/credits.js';
import * as planCompliance from './server/services/plan-compliance.js';
import { reconcileSharesUsed } from './server/services/share-credits.js';
...
/** Step-by-step trace for webhook debugging (production + CLI often share one DB → duplicates). */
function webhookTrace(event, step, detail) {
  const eventId = event?.id ? ` evt=${event.id}` : '';
  const extra = detail !== undefined && detail !== null && detail !== ''
    ? ` | ${typeof detail === 'object' ? JSON.stringify(detail) : detail}`
    : '';
  console.log(`[stripe-webhook]${eventId} [${event?.type || 'unknown'}] → ${step}${extra}`);
}

function getStripeSubscriptionPriceId(subscription) {
  const price = subscription?.items?.data?.[0]?.price;
  return stripeObjectId(price) || price?.id || null;
}

/** Period end for credits_reset_at — subscription first, then invoice fields (schedule/test-clock safe). */
function resolveBillingPeriodEndUnix(invoice, subscription) {
  if (subscription?.current_period_end) return subscription.current_period_end;
  if (invoice?.period_end) return invoice.period_end;
  const lineEnd = invoice?.lines?.data?.[0]?.period?.end;
  if (lineEnd) return lineEnd;
  return null;
}

/** True when subscription is still paid but scheduled to end (portal may set cancel_at without cancel_at_period_end). */
function isScheduledSubscriptionCancel(subscription) {
  ...
  return cancelAt > nowUnix;
}

/** ISO date when subscription ends (cancel_at preferred, then period end). */
function resolveCancelAtPeriodEndIso(subscription) {
  if (!isScheduledSubscriptionCancel(subscription)) return null;
  const unix = subscription.cancel_at ?? subscription.current_period_end;
  if (!unix) return null;
  return new Date(unix * 1000).toISOString();
}

/** Drop to free plan, clear pending cancel/downgrade, reset monthly usage. */
async function dropEntitlementToFree(entitlement, options = {}) {
  const { subscriptionStatus = 'canceled', event = null } = options;
  const freePlan = await pgClient.getPlanByCode('free');
  const resetDate = new Date().toISOString();

  if (!freePlan) {
    webhookTrace(event, 'WARN free plan not found — status/cancel fields only');
  ...
  await planCompliance.applyInstantDowngradeCompliance(entitlement.userId, 'free');
  ...
  await pgClient.resetMonthlyUsage(entitlement.userId, resetDate);
  webhookTrace(event, 'DONE dropped to free with usage reset', {
    userId: entitlement.userId,
    status: subscriptionStatus
  });
}

// Apply a plan to the user's entitlement immediately (used for upgrades and renewals)
async function applyPlanNow(plan, subscription, entitlement) {
  await pgClient.updateEntitlement(entitlement.userId, {
    plan: plan.code,
  ...
  await reconcileSharesUsed(entitlement.userId);
  console.log(`[stripe-webhook] ✅ Applied plan ${plan.code} immediately to user ${entitlement.userId}`);
}

// Schedule a plan to take effect at the next billing renewal (used for downgrades)
async function schedulePlanAtRenewal(plan, subscription, entitlement) {
  await pgClient.updateEntitlement(entitlement.userId, {
    pendingDowngradePlan: plan.code,
    stripeSubscriptionId: subscription.id,
    stripeSubscriptionStatus: subscription.status
  });
  const compliance = await planCompliance.applyInstantDowngradeCompliance(entitlement.userId, plan.code);
  console.log(`[stripe-webhook] ⏳ Scheduled plan ${plan.code} for user ${entitlement.userId} at next renewal (downgrade)`, compliance?.unpublish);
}

// Resolve new plan from the subscription's current price
async function getPlanFromSubscription(subscription) {
  const priceId = getStripeSubscriptionPriceId(subscription);
  if (!priceId) return null;
  return pgClient.getPlanByStripePrice(priceId);
}

// Used at renewal — applies whatever plan is now active on the Stripe subscription
async function syncEntitlementFromStripeSubscription(subscription, entitlement) {
  const plan = await getPlanFromSubscription(subscription);
  if (!plan) {
    console.warn('[stripe-webhook] No active local plan for Stripe price:', getStripeSubscriptionPriceId(subscription));
    return;
  }
  await applyPlanNow(plan, subscription, entitlement);
}

// WP5: Handle Stripe events
async function handleStripeEvent(event) {
  webhookTrace(event, 'START handleStripeEvent');

  // Idempotency — same evt_id stored once; prod + localhost CLI both hit same DB → second shows "duplicate"
  if (event?.id) {
    webhookTrace(event, 'STEP idempotency check');
    const firstTime = await pgClient.markStripeWebhookEventProcessed(event.id, event.type);
    if (!firstTime) {
      webhookTrace(event, 'SKIP duplicate (already in stripe_webhook_events — other listener may have processed first)');
      return;
    }
    webhookTrace(event, 'STEP idempotency OK — first delivery, continuing');
  }

  switch (event.type) {
    case 'checkout.session.completed': {
      webhookTrace(event, 'STEP enter checkout.session.completed');
      const session = event.data.object;
      const userId = session.client_reference_id || session.metadata?.userId;
      const planId = session.metadata?.planId;
      webhookTrace(event, 'STEP parsed session', { userId, planId, customer: stripeObjectId(session.customer), subscription: stripeObjectId(session.subscription) });

      if (!userId || !planId) {
        webhookTrace(event, 'ABORT missing userId or planId in session');
        return;
      }
      ...
      // Get subscription to set next billing period end (used as next reset date)
      let nextResetDate = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(); // fallback: +30 days
      ...
      webhookTrace(event, 'DONE checkout.session.completed', { userId, planId, nextResetDate, shares });
      break;
    }

    case 'customer.subscription.updated': {
      webhookTrace(event, 'STEP enter customer.subscription.updated');
      const subscription = event.data.object;

      const customerId = stripeObjectId(subscription.customer);
      webhookTrace(event, 'STEP parsed subscription', {
      ...
      if (!customerId) {
        webhookTrace(event, 'ABORT missing customer id on subscription');
        return;
      }

      webhookTrace(event, 'STEP lookup entitlement by stripe_customer_id', customerId);
      const entitlement = await pgClient.getEntitlementByStripeCustomerId(customerId);
      if (!entitlement) {
        webhookTrace(event, 'ABORT no entitlement row for customer', customerId);
        return;
      }
      webhookTrace(event, 'STEP entitlement found', {
        userId: entitlement.userId,
        plan: entitlement.plan,
        pendingDowngradePlan: entitlement.pendingDowngradePlan
      });

      // Always persist the raw Stripe status so the UI always reflects reality
      await pgClient.updateEntitlement(entitlement.userId, {
        stripeSubscriptionStatus: subscription.status
      });
      webhookTrace(event, 'STEP saved stripe_subscription_status', subscription.status);

      if (subscription.status === 'canceled' || subscription.status === 'unpaid') {
        await dropEntitlementToFree(entitlement, {
      ...
      if (subscription.status !== 'active' && subscription.status !== 'trialing') {
        webhookTrace(event, 'DONE status-only (not active/trialing)', { status: subscription.status, userId: entitlement.userId });
        break;
      }

      // Scheduled cancel — keep paid plan/limits; do not run upgrade/downgrade price sync
      if (isScheduledSubscriptionCancel(subscription)) {
        const cancelAt = resolveCancelAtPeriodEndIso(subscription);
        await pgClient.updateEntitlement(entitlement.userId, {
          stripeCancelAtPeriodEnd: cancelAt,
          stripeSubscriptionId: subscription.id,
          stripeSubscriptionStatus: subscription.status
        });
        const cancelCompliance = await planCompliance.applyInstantDowngradeCompliance(
          entitlement.userId,
          'free'
        );
        webhookTrace(event, 'DONE cancel_at_period_end scheduled', {
          userId: entitlement.userId,
          cancelAt,
          plan: entitlement.plan,
          unpublish: cancelCompliance?.unpublish
        });
        break;
      }
      ...
      if (entitlement.stripeCancelAtPeriodEnd) {
        ...
        webhookTrace(event, 'STEP cleared stripe_cancel_at_period_end (reactivated)');
      }

      webhookTrace(event, 'STEP resolve plan from Stripe price', getStripeSubscriptionPriceId(subscription));
      const newPlan = await getPlanFromSubscription(subscription);
      if (!newPlan) {
        webhookTrace(event, 'ABORT no local plan for Stripe price', getStripeSubscriptionPriceId(subscription));
        break;
      }
      webhookTrace(event, 'STEP resolved newPlan from Stripe', { code: newPlan.code, priceCents: newPlan.priceCents });

      const currentPlan = await pgClient.getPlanByCode(entitlement.plan);
      const currentPriceCents = currentPlan?.priceCents ?? 0;
      const newPriceCents = newPlan.priceCents ?? 0;
      webhookTrace(event, 'STEP compare prices', {
      ...
      if (newPriceCents > currentPriceCents) {
        webhookTrace(event, 'STEP branch UPGRADE — applyPlanNow');
        await applyPlanNow(newPlan, subscription, entitlement);
        webhookTrace(event, 'DONE upgrade applied immediately', { from: entitlement.plan, to: newPlan.code });

      } else if (newPriceCents < currentPriceCents) {
        webhookTrace(event, 'STEP branch DOWNGRADE — schedulePlanAtRenewal');
        await schedulePlanAtRenewal(newPlan, subscription, entitlement);
        webhookTrace(event, 'DONE downgrade scheduled for renewal', { from: entitlement.plan, to: newPlan.code });

      } else {
        webhookTrace(event, 'STEP branch SAME TIER — applyPlanNow');
        await applyPlanNow(newPlan, subscription, entitlement);
        webhookTrace(event, 'DONE same-tier sync', { from: entitlement.plan, to: newPlan.code });
      }
    ...
    case 'customer.subscription.deleted': {
      webhookTrace(event, 'STEP enter customer.subscription.deleted');
      const subscription = event.data.object;

      const customerId = stripeObjectId(subscription.customer);
      const entitlement = customerId
        ? await pgClient.getEntitlementByStripeCustomerId(customerId)
        : null;
      if (!entitlement) {
        webhookTrace(event, 'ABORT no entitlement for customer', customerId || subscription.customer);
        return;
      }
    ...
    case 'invoice.payment_succeeded': {
      webhookTrace(event, 'STEP enter invoice.payment_succeeded');
      const invoice = event.data.object;
      webhookTrace(event, 'STEP raw invoice fields', {
        invoiceId: invoice.id,
        customerType: typeof invoice.customer,
        customer: stripeObjectId(invoice.customer),
        subscriptionType: typeof invoice.subscription,
        subscription: stripeObjectId(invoice.subscription),
      ...
      webhookTrace(event, 'STEP parsed ids', { customerId, subscriptionId });

      if (!subscriptionId) {
        webhookTrace(event, 'ABORT not a subscription invoice (no subscription id)');
        break;
      }

      if (!customerId) {
        webhookTrace(event, 'ABORT missing customer id (check expanded customer object)');
        break;
      }

      webhookTrace(event, 'STEP lookup entitlement by stripe_customer_id', customerId);
      let entitlement = await pgClient.getEntitlementByStripeCustomerId(customerId);
      if (!entitlement) {
        webhookTrace(event, 'ABORT no entitlement row for customer', customerId);
        break;
      }

      webhookTrace(event, 'STEP entitlement found (initial)', {
        userId: entitlement.userId,
        plan: entitlement.plan,
        pendingDowngradePlan: entitlement.pendingDowngradePlan,
      ...
      entitlement = await pgClient.getEntitlement(entitlement.userId) || entitlement;
      webhookTrace(event, 'STEP entitlement refreshed from DB', {
        userId: entitlement.userId,
        plan: entitlement.plan,
        pendingDowngradePlan: entitlement.pendingDowngradePlan
      });
      ...
      try {
        webhookTrace(event, 'STEP retrieve Stripe subscription', subscriptionId);
        const stripe = await getStripeClient();
        const subscription = await stripe.subscriptions.retrieve(subscriptionId);
        webhookTrace(event, 'STEP subscription retrieved', {
          id: subscription.id,
          status: subscription.status,
          current_period_end: subscription.current_period_end,
          priceId: getStripeSubscriptionPriceId(subscription)
        });

        const periodEndUnix = resolveBillingPeriodEndUnix(invoice, subscription);
        webhookTrace(event, 'STEP resolve billing period end', {
          periodEndUnix,
          fromSubscription: subscription.current_period_end,
          fromInvoice: invoice.period_end,
          fromLine: invoice?.lines?.data?.[0]?.period?.end
        });

        if (!periodEndUnix) {
          webhookTrace(event, 'ABORT no period end on subscription or invoice');
          break;
        }

        const nextResetDate = new Date(periodEndUnix * 1000).toISOString();
        webhookTrace(event, 'STEP resetMonthlyUsage', { userId: entitlement.userId, nextResetDate });

        await pgClient.resetMonthlyUsage(entitlement.userId, nextResetDate);
        webhookTrace(event, 'STEP resetMonthlyUsage DONE');

        if (entitlement.pendingDowngradePlan) {
          webhookTrace(event, 'STEP has pending_downgrade_plan', entitlement.pendingDowngradePlan);
          const pendingPlan = await pgClient.getPlanByCode(entitlement.pendingDowngradePlan);
          if (pendingPlan) {
            webhookTrace(event, 'STEP applyPlanNow pending plan', pendingPlan.code);
            await applyPlanNow(pendingPlan, subscription, entitlement);
            webhookTrace(event, 'DONE pending downgrade applied', { plan: pendingPlan.code });
          } else {
            webhookTrace(event, 'WARN pending plan code not in plans table — sync from Stripe');
            await syncEntitlementFromStripeSubscription(subscription, entitlement);
            webhookTrace(event, 'DONE syncEntitlementFromStripeSubscription (fallback)');
          }
        } else {
          webhookTrace(event, 'STEP no pending_downgrade_plan — sync plan from Stripe');
          await syncEntitlementFromStripeSubscription(subscription, entitlement);
          webhookTrace(event, 'DONE syncEntitlementFromStripeSubscription');
        }
      } catch (error) {
        webhookTrace(event, 'ERROR in renewal try block', error?.message || error);
        console.error('[stripe-webhook] Error processing renewal:', error);
      }

      webhookTrace(event, 'DONE invoice.payment_succeeded', { invoiceId: invoice.id });
      break;
    }

    case 'invoice.payment_failed':
      webhookTrace(event, 'STEP invoice.payment_failed', { invoiceId: event.data.object?.id });
      break;

    default:
      webhookTrace(event, 'SKIP unhandled event type');
  }

  webhookTrace(event, 'END handleStripeEvent');
}

// Replit Autoscale (and most cloud hosts) sit behind a reverse proxy.
// Without this, Express sees req.protocol as 'http' even on HTTPS, and
...
app.post('/api/help/ai', aiChatRateLimiter, (req, res) => {
...
// Dashboard sale banner — plan code from env (display copy stays hardcoded in dashboard.html)
app.get('/api/dashboard/sale-plan', (req, res) => {
  const planCode = (process.env.DASHBOARD_SALE_PLAN_CODE || '').trim();
  res.json({ planCode: planCode || null });
});
...
// ---- Serve /oi-business(.html) — nav links use clean URL without .html
function serveOiBusiness(req, res) {
  const p = path.join(__dirname, 'public', 'oi-business.html');
  try {
    const html = fs.readFileSync(p, 'utf8');
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.send(html);
  } catch (e) {
    res.status(500).send('Failed to load oi-business.html');
  }
}
app.get('/oi-business.html', serveOiBusiness);
app.get('/oi-business', serveOiBusiness);
...
async function canAccessEditableProfile(req, profile) {
  if (!profile || profile.visibility === 'deleted') return false;
  if (req.user?.id && profile.userId === req.user.id) return true;

  const anonUserId = req.session?.anonymousUserId || req.cookies?.anonUserId;
  if (!anonUserId || profile.userId !== anonUserId) return false;

  const owner = await pgClient.getUser(profile.userId);
  return owner?.status === 'anonymous';
}

function denyEditableProfileAccess(req, res) {
  if (!req.user && !req.session?.anonymousUserId && !req.cookies?.anonUserId) {
    return res.status(401).json({ error: 'Authentication required' });
  }
  return res.status(403).json({ error: 'You can only access profiles owned by the current user' });
}
...
app.get("/api/profiles/:id", async (req, res) => {
  try {
    ...
    if (!(await canAccessEditableProfile(req, profile))) return denyEditableProfileAccess(req, res);
    ...
    // Widget: expose owner account status so owner_preview can show Submit vs Share
    try {
      const owner = await pgClient.getUser(profile.userId);
      profile.ownerAccountStatus = owner?.status || 'registered';
    } catch (_) {
      profile.ownerAccountStatus = 'registered';
    }
...
app.post("/api/upload/:kind(resume|attachment)/:profileId", upload.single("file"), async (req, res) => {
  try {
    ...
    if (!(await canAccessEditableProfile(req, p))) return denyEditableProfileAccess(req, res);
...
app.post("/api/upload-resume-anon", uploadRateLimiter, upload.single("file"), async (req, res) => {
  try {
    ...
    const embedWidget =
      req.body?.widget === '1' ||
      req.body?.widget === 1 ||
      String(req.body?.widget || '').toLowerCase() === 'true';

    // Set persistent anonymous user cookie (365 days, accessible to JavaScript).
    // Cross-site iframes need SameSite=None; Secure (+ Partitioned) or the cookie
    // is dropped and profile_edit receives 401 → login redirect.
    res.cookie('anonUserId', anonymousUser.id, anonUserIdSetOptions(embedWidget));
    ...
    // Widget mode: client sends widget=1 in formData to get widget-aware redirectUrl
    const isWidgetUpload = req.body.widget === '1' || req.body.widget === 'true';
    const widgetParam = isWidgetUpload ? '&widget=1' : '';
    const redirectUrl = `/profile_edit.html?id=${enrichedProfile.id}&guest=true${parseError ? '&parseError=true' : ''}${widgetParam}`;
...
app.post("/api/profiles/:id/share", requireAuth, profileRateLimiter, async (req, res) => {
  try {
    ...
    // Verify ownership - also accept anon-cookie ownership as a fallback
    // (handles the case where the profile wasn't transferred at login time)
    if (profile.userId !== userId) {
      const anonUserId = req.session?.anonymousUserId || req.cookies?.anonUserId;
      const profileOwner = await pgClient.getUser(profile.userId);
      const anonOwned = anonUserId && profile.userId === anonUserId && profileOwner?.status === 'anonymous';
      ...
      // Transfer ownership inline so share proceeds under the authenticated user
      console.log('[share] Transferring anon-owned profile', id, 'from', profile.userId, 'to', userId);
      await pgClient.updateProfile(id, { userId });
      profile.userId = userId;

      // Also transfer any files owned by the anon user
      try {
        const pool = pgClient.getPool();
        await pool.query('UPDATE files SET user_id = $1 WHERE user_id = $2', [userId, anonUserId]);
      } catch (fileErr) {
        console.error('[share] Error transferring files during inline link:', fileErr);
      }
    }
...
app.get("/api/dashboard", requireAuth, async (req, res) => {
  try {
    ...
    const planRow = await pgClient.getPlanByCode(planCode);
    const compliance = await planCompliance.getComplianceStatusForUser(userId);
    ...
    let credits = {
      ...
      // Plan info (code unchanged for Stripe/checkout; name is display-only from plans table)
      plan: planCode,
      planName: planRow?.name ?? planCode,
      cancelAtPeriodEnd: entitlement.stripeCancelAtPeriodEnd || null,
...
app.post("/api/checkout", requireAuth, async (req, res) => {
  try {
    ...
    if (!process.env.STRIPE_SECRET_KEY) {
      ...
      await reconcileSharesUsed(userId);
  ...
  try {
    ...
    try {
      ...
      const ownerEmailCandidate = [
        { source: 'profile.contact.email', value: profile.contact?.email },
        { source: 'profile.email', value: profile.email },
        { source: 'profile.person.email', value: profile.person?.email }
      ].find(candidate => typeof candidate.value === 'string' && candidate.value.trim());
      const ownerNotificationEmail = ownerEmailCandidate?.value.trim() || '';

      if (ownerNotificationEmail) {
        console.log('[booking-email] Sending owner notification', {
          profileId,
          ownerId: profile.userId,
          source: ownerEmailCandidate.source
        });
        ...
        mailer.sendMail({ to: ownerNotificationEmail, subject, html }).catch(e => console.error('[email] Failed:', e));
      } else {
        console.warn('[booking-email] No profile notification email found', {
          profileId,
          ownerId: profile.userId
        });
      }
