...
async function schedulePlanAtRenewal(plan, subscription, entitlement) {
  ...
  console.log(`[stripe-webhook] ⏳ Scheduled plan ${plan.code} for user ${entitlement.userId} at next renewal (downgrade)`);
}
...
async function handleStripeEvent(event) {
  ...
  switch (event.type) {
    case 'checkout.session.completed': {
      ...
      console.log(`[stripe-webhook] Next usage reset at: ${nextResetDate}`);
      break;
    }
    ...
    case 'customer.subscription.updated': {
      ...
      if (subscription.status === 'canceled' || subscription.status === 'unpaid') {
        // Subscription ended or payment failed — drop to free immediately
        ...
        if (freePlan) {
          ...
          console.log(`[stripe-webhook] ⚠️ Subscription ${subscription.status} — dropped user ${entitlement.userId} to free`);
        }
        break;
      }

      if (subscription.status !== 'active' && subscription.status !== 'trialing') {
        // past_due, incomplete, etc. — just status saved above, no plan change
        console.log(`[stripe-webhook] Subscription status "${subscription.status}" for user ${entitlement.userId} — no plan change`);
        break;
      }

      // Resolve the new plan from the current Stripe price
      const newPlan = await getPlanFromSubscription(subscription);
      if (!newPlan) {
        console.warn(`[stripe-webhook] Could not resolve plan for subscription ${subscription.id} — no plan change applied`);
        break;
      }

      // Resolve the current plan to compare tiers by price
      ...
      if (newPriceCents > currentPriceCents) {
        // ── UPGRADE ── apply right now so the user gets more capacity immediately
        await applyPlanNow(newPlan, subscription, entitlement);
        console.log(`[stripe-webhook] ⬆️  Upgrade: ${entitlement.plan} → ${newPlan.code} (user ${entitlement.userId}) — applied immediately`);

      } else if (newPriceCents < currentPriceCents) {
        // ── DOWNGRADE ── schedule for next renewal so the user keeps their paid capacity
        // for the rest of the current billing period; limits change when usage resets
        await schedulePlanAtRenewal(newPlan, subscription, entitlement);
        console.log(`[stripe-webhook] ⬇️  Downgrade: ${entitlement.plan} → ${newPlan.code} (user ${entitlement.userId}) — scheduled for next renewal`);

      } else {
        // Same price tier (e.g. interval change, promo, trial → active) — sync immediately
        await applyPlanNow(newPlan, subscription, entitlement);
        console.log(`[stripe-webhook] 🔄 Same-tier change: ${entitlement.plan} → ${newPlan.code} (user ${entitlement.userId}) — applied immediately`);
      }
    ...
    case 'invoice.payment_succeeded': {
      // New billing period started — reset usage and apply any pending plan change
      const invoice = event.data.object;
      const customerId = invoice.customer;

      // Only process subscription invoices (not one-off charges)
      if (!invoice.subscription) {
        console.log('[stripe-webhook] invoice.payment_succeeded: not a subscription invoice, skipping');
        break;
      }
      ...
      if (entitlement) {
        try {
          ...
          const rawSubId = invoice.subscription && typeof invoice.subscription === 'string'
            ? invoice.subscription
            : entitlement.stripeSubscriptionId;
          const resolvedSubId = typeof rawSubId === 'string' ? rawSubId : rawSubId?.id;

          if (resolvedSubId) {
            const subscription = await stripe.subscriptions.retrieve(resolvedSubId);

            if (subscription && subscription.current_period_end) {
              const nextResetDate = new Date(subscription.current_period_end * 1000).toISOString();

              // 1. Reset all usage counters for the new billing period
              await pgClient.resetMonthlyUsage(entitlement.userId, nextResetDate);
              console.log(`[stripe-webhook] ✅ Usage reset for user ${entitlement.userId}, next reset: ${nextResetDate}`);

              // 2. Apply pending downgrade if one was scheduled, otherwise sync from Stripe
              if (entitlement.pendingDowngradePlan) {
                ...
                if (pendingPlan) {
                  ...
                  console.log(`[stripe-webhook] ⬇️  Pending downgrade applied: → ${pendingPlan.code} for user ${entitlement.userId}`);
                } else {
                  // Pending plan no longer exists — fall back to syncing from Stripe
                  console.warn(`[stripe-webhook] Pending downgrade plan "${entitlement.pendingDowngradePlan}" not found — syncing from Stripe instead`);
                  await syncEntitlementFromStripeSubscription(subscription, entitlement);
                }
              } else {
                // No pending change — just re-sync plan limits from Stripe (handles any
                // direct Stripe-side changes not caught by customer.subscription.updated)
                await syncEntitlementFromStripeSubscription(subscription, entitlement);
                console.log(`[stripe-webhook] ✅ Plan re-synced for user ${entitlement.userId} after renewal`);
              }
            }
          }
        } catch (error) {
